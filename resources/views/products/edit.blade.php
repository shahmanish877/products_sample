                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-10">
                                    <textarea id="summary" class="form-control @error('summary') is-invalid @enderror" name="summary">{{ old('summary', $product->summary) }}</textarea>
                                    @error('summary')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-10">
                                    <input id="owner_email" type="text" class="form-control @error('owner_email') is-invalid @enderror" name="owner_email" value="{{ old('owner_email', $product->owner_email) }}" required autocomplete="owner_email" autofocus>
                                    @error('owner_email')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-10">
                                    <?php
                                        $cat_arr_old = $product->category;
                                        if(old('category'))
                                            $cat_arr_old = implode(', ', old('category'));
                                    ?>
                                    ...
                                    <select id="category" class="form-control @error('category') is-invalid @enderror" name="category[]" multiple="multiple" required>
                                        @foreach($cat_arr as $cat)
                                            <?php echo $cat ."===".strpos($cat_arr_old, $cat);?>
                                            <option value="{{$cat}}"  {{ ( stripos($cat_arr_old, $cat) > -1 ) ? 'selected':'' }}>{{$cat}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-10">
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" value="Physical" name="type" id="Physical" checked {{ old('type', $product->type) == 'Physical' ? 'checked' : ''  }}>
                                        <label class="form-check-label" for="Physical">
                                            Physical
                                        </label>
                                    </div>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" value="Digital" name="type" id="Digital" {{ old('type', $product->type) == 'Digital' ? 'checked' : ''  }}>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-10">
                                    <input id="expiry_date" type="date" class="form-control @error('expiry_date') is-invalid @enderror" name="expiry_date" value="{{ old('expiry_date', $product->expiry_date) }}" autocomplete="expiry_date" autofocus>
                                    @error('expiry_date')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
